...
Given the evidence presented in Ding et al. (2026), that the Tobacco Root Mountains are rotated relative to stable Laurentia, the previous Tobacco Root Gunbarrel dikes pole is no longer included in the compilation. The previous 1448±49 Ma Tobacco Root Dykes pole is no longer either as it is part of the same rotated terrane. The Tsezotene Sills Combined pole is also removed as it was interpreted by Ding et al. (2026) to also suffer from rotation.
...
Otherwise, there is impressive consistency between different poles for much of the record. Take for example the comparison between the ca. 719 Ma Franklin LIP pole and the ca. 755 Ma Chuar pole that despite being from very different parts of Laurentia and from very different lithologies have a very similar pole position. One can similarly point to the similarity between the ECMB and Cleaver dikes in the Paleoproterozoic, and the Cardenas pole and late stage Midcontinent Rift volcanics,